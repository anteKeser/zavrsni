...
class FirebaseSensorRepositoryImpl : SensorRepository {
    ...
    override suspend fun getHistoricalData(hoursBack: Int): List<SensorData> {
        return try {
            ...
            val snapshot =
                historyDataRef.orderByKey().startAt(thresholdMs.toString()).valueEvents.first()
    ...
    override suspend fun clearOldLogs(olderThanDays: Int): Int {
        return try {
            val nowMs = Clock.System.now().toEpochMilliseconds()
            val thresholdMs = nowMs - (olderThanDays * 24 * 60 * 60 * 1000L)

            val snapshot =
                historyDataRef.orderByKey().endAt(thresholdMs.toString()).valueEvents.first()

            val staleKeys = snapshot.children.mapNotNull { it.key }
            staleKeys.forEach { key -> historyDataRef.child(key).removeValue() }
    ...
    override suspend fun getLogCount(): Int {
        return try {
            val snapshot = historyDataRef.valueEvents.first()
